    ...
    list.innerHTML = filtered.map(t => {
        ...
        const photoHTML = t.photoUrl ? `
            <div class="ticket-attachment">
                <a href="${t.photoUrl}" target="_blank">
                    <img src="${t.photoUrl}" alt="Attachment">
                </a>
            </div>` : '';
        ...
        const commentsHTML = comments.map(c => `
            <div class="comment-item">
                <div class="comment-meta">
                    ...
                    <span>${c.createdAt?.toDate ? c.createdAt.toDate().toLocaleString() : new Date(c.createdAt).toLocaleString()}</span>
                </div>
        ...
        return `<div class="ticket-card ticket-status-${t.status}">
            ...
            <div class="ticket-comments-section">
                ...
                <div class="comment-input-row">
                    <input type="text" class="comment-input" id="input-${t.id}" placeholder="新增留言或回覆...">
                    <button class="comment-submit-btn" onclick="addTicketComment('${t.id}')">送出</button>
                </div>` : ''}
            </div>
        </div>`;
    }).join('');
...
window.addTicketComment = async (id) => {
    ...
    try {
        ...
        const comments = ticketSnap.data().comments || [];
        comments.push(newComment);

        await updateDoc(ticketRef, { comments: comments });
        input.value = '';
    } catch (err) {
        console.error(err);
        ...
        return `
            <div class="discussion-card ${d.isDone ? 'is-done' : ''}">
                ...
                <div class="discussion-actions">
                    ${!d.isDone && canManage ? `<button class="disc-btn disc-btn-done" onclick="resolveDiscussion('${d.id}')">?? 結案</button>` : ''}
                    ${canEdit ? `<button class="disc-btn disc-btn-edit" onclick="editDiscussion('${d.id}', \`${d.title.replace(/'/g, "\\'")}\`, \`${(d.description || '').replace(/'/g, "\\'")}\`)">?? 編輯</button>` : ''}
                    ${canManage ? `<button class="disc-btn disc-btn-delete" onclick="deleteDiscussion('${d.id}')">??? 刪除</button>` : ''}
                </div>
            </div>
        `;
    }).join('');
